...
export default function EviTrack() {
  ...
  // The deployment may hold a Groq key, a Gemini key, or neither. Ask the
  // server which one it can actually reach and default the picker to that,
  // instead of always offering Gemini and failing when Groq is configured.
  ...
  useEffect(() => {
    ...
    fetch("/api/v1/llm/status")
      ...
      .then((s) => {
        ...
        setLlmStatus(s);
        if (s.provider === "groq") setSelectedModel("openai/gpt-oss-120b");
      })
      .catch(() => {});
    return () => {
      cancelled = true;
    ...
    <section className="evitrack">
      ...
      {llmStatus && !llmStatus.configured && (
        <div className="alert info">
          <span>
            Evidence search works, but AI insight summaries are switched off:
            no LLM key is configured on the server.
          </span>
        </div>
      )}
