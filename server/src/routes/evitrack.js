...
import {
  ...
  listEvidence,
  saveEvidence,
} from "../services/evitrack/evidenceRepository.js";
...
evitrackRouter.get("/search", async (req, res, next) => {
  try {
    ...
    return res.json({
      ...
      // Without a database the results are live but not stored; say so rather
      // than implying a save happened.
      persisted: evidenceStorageAvailable(),
  ...
  try {
    ...
    if (!evidenceStorageAvailable()) {
      return res.status(503).json({
        detail:
          "Evidence storage is unavailable: no database is configured. " +
          "Search still works, but records cannot be saved.",
      });
    }
...
evitrackRouter.get("/health", (_req, res) => {
  return res.json({
    module: "evitrack",
    status: "ok",
    storage: evidenceStorageAvailable() ? "database" : "none (search only)",
  });
});
